  ...
  return (
    <div className="min-h-screen flex flex-col pb-28">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-3 py-5 space-y-5">
        <RosterCard roster={rosters.glorious_service} hasEditAccess={authRole === 'master'} onCellChange={handleCellChange} savedRows={savedRosters.glorious_service.rows} />
      </main>
      <ControlDock hasEditAccess={authRole === 'master'} unsavedCount={unsavedCount} onSave={handleSave} onCancel={() => setRosters(JSON.parse(JSON.stringify(savedRosters)))} onReset={() => {}} onDownload={() => exportRosterPNG(rosters.glorious_service, isDark)} isSaving={isSaving} />
    </div>
  );
}
